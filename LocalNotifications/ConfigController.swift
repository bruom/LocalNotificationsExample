...
class ConfigController: UITableViewController, UIPickerViewDelegate, UIPickerViewDataSource {
...
//    }
    ...
    @IBAction func sendButton(_ sender: Any) {
        ...
        var desc = self.descriptionTextField.text ?? "Descricao"
        if desc.count < 1 {
            desc = "Descricao"
        }
        ...
        notificationCenter.getNotificationSettings { (settings) in
            if settings.authorizationStatus == .authorized {
                ...
                let request = UNNotificationRequest(identifier: "config", content: content, trigger: trigger)
